        ...
        <>
            {
                ...
                <div className='group w-48 sm:w-48 lg:w-80 p-3 h-fit rounded-md group relative hover:border-gray-200 border-[0.3ex] border-white duration-500 transition-all'>
                    <DiscountLabel discount={product.discount} labelClassName={`fill-[#509abd] m-1.5 w-11 lg:w-[10ex] absolute`} discountClassName='text-white mt-[2.7ex] ml-[4.3ex] text-[0.7ex] lg:text-[1.4ex] lg:mt-[1.7ex] lg:ml-[3.5ex]' />
                    <div className='flex flex-col gap-3'>
                        <img
                            ...
                            alt='Product'
                        />
                        <StarRating starClassName='size-4' defaultRating={product.rating.rate} />
                        <hr />
                        <Link to={`../product/${product.id}`} className='text-sm lg:text-md font-semibold h-11 line-clamp-2 hover:text-primary transition-all duration-300'>{product.title}</Link>
                        <div />
